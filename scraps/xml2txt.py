...
with open('/media/e/weizhang/data/0625/jsonlist') as jslist:
    lines=jslist.readlines()
ff=open('/media/e/weizhang/data/0625/txtlist.txt','w')
for line in lines:
    name = line[:-1][find_last(line[:-1], '/') + 1:-9]
    f1=open(line[:-1])
    x=json.load(f1)
    obj=x['objects']
    img=cv2.imread('/media/e/weizhang/data/0625/mark/'+na me+'.jpg')
    ...
    f=open('/media/e/weizhang/data/0625/txt/'+name+'.txt','w')
    f.write(str(len(obj))+'\n')
    for x1 in obj:
